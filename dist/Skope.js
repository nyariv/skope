    ...
    for (var i = 0; i < str.length; i++) {
        ...
        if (comment) {
            ...
            if (quote) {
                ...
                else if (closings$1[currentEnclosure] === char && !enclosures.length) {
                ...
                else if (closings$1[enclosures[enclosures.length - 1]] === char) {
                    enclosures.pop();
                    strRes.push(char);
                }
...
function createErrorCb(el) {
    return (err) => createError(err?.message, el);
}
...
function getScope(skope, element, subs, vars = {}, root) {
    let scope = skope.getStore(element, 'scope');
    if (root) {
...
function getScopes(skope, element, subs = [], newScope) {
    if (!element)
        return [];
    const scope = newScope === undefined ? skope.getStore(element, 'scope') : getScope(skope, element, subs, newScope);
    const scopes = skope.getStore(element, 'scopes') || [];
    if (scopes.length) {
        return [...scopes, scope];
    }
    if (scope)
...
function walkTree(skope, element, parentSubs, ready, delegate, skipFirst) {
    ...
    if (element instanceof Element) {
        ...
        for (const att of element.attributes) {
            if (att.nodeName.startsWith(':')) {
                attributeDirective(skope, element, att, currentSubs, ready);
            }
...
function createClass(sanitizer) {
    ...
    class ElementCollection {
        ...
        sort(callback) {
            if (!callback) {
                ...
                const sorted = this.$elements.sort(callback);
        ...
        delegate() {
            ...
            let del = getStore(that, 'delegate');
            if (del)
                return del;
            ...
            if (!that)
                return defaultDelegateObject;
            del = {
                on(elem, ev, handler) {
                    const evs = events.get(elem) || new Map();
                    events.set(elem, evs);
                    const subs = [];
                        ...
                        if (!started.has(event)) {
                            const evcb = (e) => {
                                ...
                                listeners.forEach((handlers, i) => {
                                    if (eq.stoppedPropagation)
                                        return;
                                    handlers.forEach((handle) => {
                                        if (eq.stoppedImmediatePropagation)
                                            return;
                                        handle(eq);
                                    });
                                });
        ...
        val(set) {
            ...
            if (elem instanceof HTMLSelectElement) {
                const res = [...elem.options].filter((opt) => opt.selected).map((opt) => opt.value);
                if (elem.multiple) {
                    const ret = getStore(elem, 'multiSelect', []);
                    ret.length = 0;
                    ret.push(...res);
                    return ret;
                }
        ...
        label(set) {
            ...
            if (!this.get(0))
                return null;
            const get = (test) => test && test.length && test;
            return get(this.attr('aria-label'))
                || get(get(this.attr('aria-labelledby')) && wrap(`#${this.attr('aria-labelledby')}`, this).label())
                || get(get(this.attr('id')) && wrap(`label[for="${this.attr('id')}"]`, this).label())
                || get(this.attr('title'))
        ...
        data(key, set) {
            ...
            if (!arr(this)[0])
                return null;
            const data = getStore(arr(this)[0], 'data') || {};
        ...
        once(identifier) {
            ...
            const res = filter(this, (elem, i) => {
                const once = getStore(elem, 'once', new Set());
    ...
    function wrapEvent(originalEvent) {
        ...
        let { currentTarget } = originalEvent;
        ...
        for (const p in originalEvent) {
            if (props[p])
                continue;
            if (typeof originalEvent[p] === 'function') {
                const fn = originalEvent[p];
                props[p] = {
                    get: () => (...args) => fn.call(originalEvent, ...args),
                };
            }
            else if (p !== 'isTrusted') {
                props[p] = {
    ...
    function wrap(selector, context) {
        ...
        if (!context)
            return new ElementCollection();
        const doc = context instanceof ElementCollection ? ownerDoc(context) : context;
        if (!doc)
            return new ElementCollection();
        const selectors = selector instanceof Array ? selector : [selector];
        const $context = context instanceof ElementCollection ? context : new ElementCollection(doc.documentElement);
        const elems = new Set();
        let doFilter = true;
        let doSort = selectors.length > 1;
        if ((selectors.length === 1 && $context.length === 1 && selectors[0] === arr($context)[0]) || $context === selector)
            return $context;
    ...
    function filter(elems, selector) {
        if (!selector)
            return new ElementCollection(...arr(elems).filter((elem) => elem instanceof Element));
        if (typeof selector === 'function') {
    ...
    function propElem(collection, propArg, selector, multiple, includeFirst, stopAt, reverse) {
        const res = new Set();
        const cache = new Set();
        const sSelector = typeof selector === 'string' ? selector : undefined;
        const is = (elem, sel) => {
            if (!(elem instanceof Element))
                return false;
            if (!sel)
    ...
    function from(object) {
        if (typeof object !== 'object' || !object)
            return new ElementCollection();
        if (object instanceof ElementCollection)
            return object;
        let size = 0;
    ...
    function getStore(elem, store, defaultValue) {
        ...
        const types = elementStorage.get(elem);
...
function modelDirective(skope) {
    return {
        ...
        callback: (exec, scopes) => {
            ...
            const $el = skope.wrapElem(el);
            let last = !isContentEditable ? $el.val() : $el.html();
...
function initialize(skope) {
    ...
    ElementCollection.prototype.html = function h(content) {
        ...
        let currentSubs = skope.getStore(elem, 'currentSubs', []);
        ...
        if (elem instanceof HTMLIFrameElement) {
            ...
            const recurse = (el) => {
                if (!el || !el.parentElement || el.matches('[skope]') || el.hasAttribute('s-detached'))
                    return [];
    ...
    const directives$1 = directives(skope);
    for (const dir in directives$1) {
...
class Skope {
    ...
    init(elem, component, alreadyPreprocessed = false) {
        const subs = [];
    ...
    processHTML(skope, elem, subs, delegate, skipFirst = false) {
