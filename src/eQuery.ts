...
type EqEventListenerAny = EqEventListener<any>;
...
export default function createClass(sanitizer: () => IHTMLSanitizer): {
  ...
  class ElementCollection implements IElementCollection {
    constructor(item?: number | Element, ...items: Element[]) {
      arrs.set(this, new Array(item, ...items) as Element[]); // eslint-disable-line
    }
    ...
    delegate(): DelegateObject {
      ...
      const started = new Map<string, () => void>(); // eslint-disable-line
     ...
     * Element.value
     */
    val(set?: boolean | string | number | string[]): string | number | string[] | boolean | FileList | undefined {
    ...
    vals(set?: { [name: string]: boolean | string | number | string[] }): { [name: string]: boolean | string | number | string[] | FileList } {
      ...
      const res: { [name: string]: boolean | string | number | string[] | FileList } = {};
      if (set === undefined) {
        $elems.forEach((elem: HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement) => {
          ...
          if (elem instanceof HTMLInputElement && elem.type === 'radio') {
            res[elem.name] = res[elem.name]; // eslint-disable-line
            if (elem.checked) {
    ...
    label(set?: string): string | this {
      ...
      const get = (test: any) => test && test.length && test;
    ...
    data(key: string | { [data: string]: any }, set?: any): unknown {
      if (objectOrProp(key, set, (k, v) => {
        this.forEach((elem) => {
          const data = getStore<any>(elem, 'data', {});
          data[k] = v;
        });
      })) return this;
      ...
      const data = getStore<any>(arr(this)[0], 'data') || {};
     ...
     */
    nextUntil(selector?: string | Element, filterArg?: string): IElementCollection {
     ...
     * Element.previousElementSibling
     */
    prevUntil(selector?: string | Element, filterArg?: string): IElementCollection {
  ...
  function wrapEvent<T extends Event>(originalEvent: (T | (T & EqEvent))): T & EqEvent {
    ...
    for (const p in originalEvent) {
      if (props[p]) continue;
      if (typeof (originalEvent as any)[p] === 'function') {
        const fn = (originalEvent as any)[p];
        props[p] = {
          get: () => (...args: any[]) => fn.call(originalEvent, ...args),
        };
      } else if (p !== 'isTrusted') {
        props[p] = {
          value: (originalEvent as any)[p],
        };
      }
    }
  ...
  function wrap(selector: WrapType, context: IElementCollection | HTMLDocument): IElementCollection {
    ...
    const doc = context instanceof ElementCollection ? ownerDoc(context) : context as HTMLDocument;
    if (!doc) return new ElementCollection();

    const selectors: Array<unknown> = selector instanceof Array ? selector : [selector];
   ...
   * @returns {Set<Element>}
   */
  function propElem(collection: IElementCollection | Array<Element>, propArg: string, selector?: string, multiple?: boolean, includeFirst?: boolean, stopAt?: string | Element, reverse?: boolean): Set<Element> {
    const res = new Set<Element>();
    const cache = new Set<Element>();
    const sSelector = typeof selector === 'string' ? selector : undefined;
    const is = (elem: Element, sel?: string | Element) => {
   ...
   */
  function getStore<T>(elem: Node, store: string, defaultValue?: T): T | undefined {
